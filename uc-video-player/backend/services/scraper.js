 ...
 */
export async function getHlsResolutions(m3u8Url) {
  try {
    ...
    if (!content.includes('#EXT-X-STREAM-INF')) {
      // It's a media playlist (single quality), not a master playlist
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('#EXT-X-STREAM-INF:')) {
        // Find RESOLUTION=1920x1080 or similar
    ...
    // Sort by height/bandwidth descending
...
export async function scrapeUrl(targetUrl) {
  try {
    ...
    if (/\.(mp4|mkv|webm|avi|m3u8)(\?.*)?$/i.test(cleanTargetUrl)) {
      ...
      const filename = cleanTargetUrl.split('/').pop()?.split('?')[0] || 'Direct Stream';
      ...
      try {
        ...
        $('a').each((_, elem) => {
          ...
          if (href.startsWith('magnet:')) {
            ...
            addVideo(href, text || href.split('/').pop()?.split('?')[0], type);
          }
        });
